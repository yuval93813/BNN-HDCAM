...
def run_last_layer(weights, activation, MnistLable, max_hd):
    ...
    for weight in weights:
        ...
        for hd in range(max_hd+1):
            ...
            resultFinel = 1 if ((32-result1low) <= hd or (32-result1high) <= hd or (32-result2low) <= hd or (32-result2high) <= hd) else 0
...
for active in activation:
    correct, top_2_correct = run_last_layer(weights, [active[1]], active[0], 13)
...
print(f"Total correct predictions: {count_res}")
print(f"Total top 2 accuracy: {top_2_count}")
print(labelmiss)
